...
import time
import copy
import Acciones.colisiones as colisiones

class Pez:
    def __init__(self):
        #inicializmos variables para el lanzamiento
        ...
        #calculamos los vertices del cuerpo
    ...
    def actualizar_posicion(self): 
        if self.esta_en_movimiento:
            ...
            #hacemos un tiro parabolico (creo que si se llama asi)
            ...
            #validamos si toco el suelo
            if self.posicion[1] <= 0.01:
                self.esta_en_movimiento = False
                self.toco_suelo = True
        
        else:    
            #si no se lanza se mantiene en la mano de heisenpurr             
            from clasePrincipalLuis import heisenpurr
    ...
    def render_pez(self):
        ...
        for vertex in self.vertices_cola:
            glVertex3fv(vertex)
        glEnd()
